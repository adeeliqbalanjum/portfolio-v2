  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (reduced) {
        ...
        gsap.set('.dx-reveal, .dx-case-card, .dx-index-row', { opacity: 1, y: 0, clearProps: 'transform' });
        return;
      }
      ...
      gsap.utils.toArray('.dx-case-card').forEach((card, index) => {
        const media = card.querySelector('.dx-case-image');
        gsap.fromTo(
          card,
          { y: 70, opacity: 0, scale: 0.97, rotate: index % 2 ? 1.2 : -1.2 },
          {
            y: 0,
            opacity: 1,
            scale: 1,
    ...
    <main className="dx-site" id="home" ref={rootRef}>
      ...
      <section className="dx-work dx-section" id="work">
        ...
        <div className="dx-container">
          ...
          <div className="dx-case-stack">
            {showcaseProjects.map((project, index) => (
              <article className="dx-case-card dx-glass" style={{ '--card-offset': `${index * 10}px` }} key={project.slug}>
